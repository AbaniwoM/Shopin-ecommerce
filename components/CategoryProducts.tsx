...
const CategoryProducts = ({ categories, slug }: Props) => {
  ...
  const fetchProducts = async (categorySlug:string) => {
    ...
    try {
      const query = `
      *[_type == 'product' && references(*[_type == "category" && slug.current ==
    ...
    <div className="py-5 flex flex-col md:flex-row items-start gap-5">
      <div className="flex flex-col md:min-w-40 border">
        {categories?.map((item) => (
            ...
            text-darkColor shadow-none hover:bg-shop_orange
            hover:text-white font-semibold hoverEffect border-b
            last:border-b-0 transition-colors capitalize ${item?.slug?.
            current === currentSlug && "bg-shop_orange text-white border-shop_orange"}`}
            >
